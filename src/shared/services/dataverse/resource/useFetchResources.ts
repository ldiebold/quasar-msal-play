...
import { DataversePopulate } from '../types/DataversePopulate'
import { DataverseFieldsSelect } from '../plugin/DataverseFieldsSelect'
import { DataverseFiltersObject } from '../types/DataverseFilters'
...
export interface UseFetchResourcesOptions<ModelType extends Model> {
  populate?: MaybeComputedRef<DataversePopulate<ModelType>>
  sort?: MaybeComputedRef<string[]>
  fields?: MaybeComputedRef<DataverseFieldsSelect<ModelType>>
  filters?: MaybeComputedRef<DataverseFiltersObject<ModelType>>
...
export default function useFetchResources<ModelType extends typeof Model> (
  ...
  async function fetch () {
    await rest.execute('get', {
      populate: options.populate ? resolveUnref(options.populate) as DataversePopulate<InstanceType<ModelType>> : undefined,
      filters: options.filters ? resolveUnref(options.filters) as DataverseFiltersObject<InstanceType<ModelType>> : undefined,
